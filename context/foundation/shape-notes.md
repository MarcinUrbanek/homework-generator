...
checkpoint:
  ...
  gray_areas_resolved:
    ...
    - topic: primary persona scope
      decision: primary-school classroom math teacher in Poland
    - topic: differentiating insight
      decision: existing sources provide too few distinct exercises at equivalent difficulty
    - topic: authentication strategy
      decision: email and password
    - topic: role model
      decision: teachers and students
    - topic: account creation
      decision: teachers self-register and invite students
    - topic: MVP scope
      decision: full teacher-to-student flow retained with a two-week estimate
    - topic: assignment distribution
      decision: platform automatically builds a distinct, comparably difficult set per student
    - topic: passing threshold
      decision: a score of at least 50 percent is passed
    - topic: score calculation
      decision: correct answers divided by assigned exercises
    - topic: result release
      decision: platform releases pass or fail immediately without teacher approval
    - topic: product surface
      decision: web app
    - topic: target user scale
      decision: dozens to one hundred users; domain rules remain unchanged at one hundred times that scale
    - topic: delivery timing
      decision: two weeks of after-hours work with no hard deadline
  frs_drafted: 13
...
I would like to create a platform where math teachers in poland could create homework for primary school students.
...
Existing worksheets and exercise banks provide too few distinct exercises at reliably equivalent difficulty.

The validation, difficulty-grouping, assignment, and passing rules remain unchanged at roughly 100 times the initial target scale.
...
Acknowledged on 2026-09-14: 2-week MVP requires sustained dedication; user accepted.
...
- The user proposed OpenRouter for exercise generation and a second model for answer verification. This is a downstream technical-selection input, not a PRD commitment.
